      ...
      return Container(
        ...
        child: Column(
          children: [
            ...
            HeaderText(
                text: MyStrings.yourOfferAmount.tr,
              ...
              children: [
                  ...
                  child: Center(
                    child: Text(
                      MyStrings.recommendPrice.rKv({
                        "priceKey":
                            "${controller.defaultCurrencySymbol}${StringConverter.formatNumber(widget.ride.minAmount.toString())}",
                        "distanceKey":
                            "${widget.ride.distance}${MyStrings.km.tr}".tr,
                      }).tr,
                            ...
                            MyDebouncer(
                              ...
                              if (val.isNotEmpty) {
                                ...
                                if (enterValue.toPrecision(0) >=
                                        (double.tryParse(widget.ride.minAmount
                                                .toString()) ??
                                            0) &&
                                    (enterValue.toPrecision(0)) <
                                        (double.tryParse(widget.ride.maxAmount
                                                .toString()) ??
                                            0)) {
                                  printX(
                                      'valid ${widget.ride.minAmount} ${widget.ride.maxAmount}');
                ...
                RoundedButton(
                  ...
                  press: () {
                    ...
                    if (enterValue.toPrecision(0) >=
                            (double.tryParse(
                                    widget.ride.minAmount.toString()) ??
                                0) &&
                        (enterValue.toPrecision(0)) <
                            (double.tryParse(
                                    widget.ride.maxAmount.toString()) ??
                                0)) {
                      Get.back();
                      widget.onBtnPress();
                    } else {
                      CustomSnackBar.error(
                        errorList: [
                          '${MyStrings.pleaseEnterMinimum.tr} ${controller.defaultCurrencySymbol}${StringConverter.formatNumber(widget.ride.minAmount ?? '')} ${MyStrings.to.tr} ${controller.defaultCurrencySymbol}${StringConverter.formatNumber(widget.ride.maxAmount ?? '')}'
                        ],
                      );
                    }
                  },

...
class VehicleAlreadyVerifiedWidget extends StatefulWidget {
  ...
  @override
  State<VehicleAlreadyVerifiedWidget> createState() =>
      _VehicleAlreadyVerifiedWidgetState();
}

class _VehicleAlreadyVerifiedWidgetState
    extends State<VehicleAlreadyVerifiedWidget> {
  @override
                      ...
                      child: FittedBox(
                        child: Column(
                          ...
                          children: [
                            MyImageWidget(
                                imageUrl:
                                    controller.selectedService.imageWithPath ??
                                        '',
                                height: 40,
                                width: 60,
                                radius: 1),
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                          ...
                          borderRadius:
                              BorderRadius.circular(Dimensions.mediumRadius),
                          border: Border.all(
                              color: MyColor.primaryColor, width: 1.5)),
                      child: FittedBox(
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            MyImageWidget(
                                imageUrl:
                                    controller.selectedBrand.imageWithPath ??
                                        '',
                                height: 40,
                                width: 60,
                                radius: 1),
                            spaceDown(Dimensions.space5),
                            FittedBox(
                              child: Text(controller.selectedBrand.name ?? '',
                                  style: regularDefault.copyWith()),
                            )
                          ],
                        ),
                      ...
                      return Container(
                        ...
                        child: controller.pendingData[index].type == "file"
                            ? Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(controller.pendingData[index].name ?? '',
                                  ...
                                  GestureDetector(
                                    onTap: () {
                                      String url =
                                          "${UrlContainer.domainUrl}/${controller.path}/${controller.pendingData[index].value.toString()}";
                                      printX(url);
                                      showDialog(
                                        context: context,
                                        builder: (context) {

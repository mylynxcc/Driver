...
import 'package:ovoride_driver/data/model/ride/new_ride_response_model.dart';
...
class NewRideController extends GetxController {
  ...
  Future<void> getNewRideList({int? p, bool shouldLoading = true}) async {
    ...
    try {
      ...
      if (responseModel.statusCode == 200) {
        NewRideListResponseModel model = NewRideListResponseModel.fromJson(
  ...
  Future<void> createBid(String id) async {
    ...
    try {
      ResponseModel responseModel =
          await repo.createBid(rideId: id, amount: mainAmount.toString());
      if (responseModel.statusCode == 200) {
  ...
  updateMainAmount(double amount) {
    mainAmount = amount;
    amountController.text =
        StringConverter.formatNumber(mainAmount.toString(), precision: 0);
    update();
  }
  ...
  removeMainAmount(double amount) {
    if (mainAmount > 0) {
      mainAmount -= amount;
      amountController.text =
          StringConverter.formatNumber(mainAmount.toString());
      update();
    }
  }
}

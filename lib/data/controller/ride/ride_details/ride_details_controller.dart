    ...
    try {
      ...
      if (responseModel.statusCode == 200) {
        ...
        if (model.status == MyStrings.success) {
          ...
          printX(
              "----999999999999999999999-- ${ride.paymentStatus} ${ride.paymentType} ${ride.status}");
...
//
  ...
  Future<void> openGoogleMaps() async {
    final String pickup = UrlContainer.googleDirection
        .replaceAll("{lat}", "${ride.pickupLatitude},")
        .replaceAll("{long}", ride.pickupLongitude.toString());
    final String destination = UrlContainer.googleDirection
        .replaceAll("{lat}", "${ride.destinationLatitude},")
        .replaceAll("{long}", ride.destinationLongitude.toString());

    final String url = UrlContainer.googleDirection
        .replaceAll("{lat}", "${ride.pickupLatitude},")
        .replaceAll("{long}", ride.pickupLongitude.toString());
    ...
    if (await canLaunchUrl(Uri.parse(
        ride.status == AppStatus.RIDE_RUNNING ? destination : pickup))) {
      await launchUrl(
          Uri.parse(
              ride.status == AppStatus.RIDE_RUNNING ? destination : pickup),
